...
Exclude participants with a dprime below zero

```{r dprime}

#calculate hit rate by participant

dprime_df = retrieval[, .(HR = mean(correct)), by = participant]
dprime_df[, FA := 1-HR]
#there aren't any 1s or 0s to correct
dprime_df[, dprime := qnorm(HR) - qnorm(FA)] 

exclude_dprime = dprime_df[dprime <= 0]$participant #more than half their trials were of abnormal latency or they didn't finish

retrieval = retrieval[!participant %in% c(exclude_dprime)]
...
#csv that includes all trials for each participant above dprime threshold (even abnormal trial latencies)
extra_retrieval_full = data[!participant %in% c(exclude_dprime) & blockcode == "retrieval" ]

# label conditions
extra_retrieval_full$condition = "random" # batches 1-8
extra_retrieval_full[batch %in% c(9:16)]$condition = "software" # batches 9-16
write.csv(extra_retrieval_full, file = "extra_retrieval_full.csv")


#number of participants per condition
length(unique(retrieval[condition == "random"]$participant))
length(unique(retrieval[condition == "software"]$participant))
