...
all_cohensd_random_HR = cohen.d(
  ...
  paired = T) # 0.94
all_cohensd_random_HR$estimate
...
all_cohensd_software_HR = cohen.d(
  ...
  paired = T) # 0.86
all_cohensd_software_HR$estimate
...
# power analysis for random (n = 10.9)
pwr.t.test(d = all_cohensd_random_HR$estimate, # effect size of random
           sig.level = .05, power = .8, type = ("paired"))

# power analysis for software (n = 12.7)
pwr.t.test(d = all_cohensd_software_HR$estimate, # effect size of software
           sig.level = .05, power = .8, type = ("paired"))

```

Overall, these analyses don't show a benefit towards the software-- even showing a slight benefit towards the random condition.
...
SelectRandomOld <- function(n=10){
  # takes n number of images per group of stimuli, in the random table
  # returns rows where these images were seen in the encoding phase
  
  tmp = as.data.table(matrix(nrow=0, ncol=ncol(random)))
  names(tmp) = names(random)
  
  for(b in 1:8){
    # for each batch, sample a certain number of stimuli w/o overlap
...
SelectRandomAll <- function(n=10){
  # takes n number of images per group of stimuli, in the random table
  # returns rows where these images were seen in either the encoding or retrieval phase
...
RandomSelectionEffectSize <- function(n_stim = 10, n_iter = 100){
  effect_list_random <- c()

  # simulate many times
  for(i in 1:n_iter){
    # get mean accuracy for 10 random stimuli
    randstim = SelectRandomOld(n=n_stim)
    randstim1 = randstim[, .(mean = mean(correct)), 
    ...
    goodparticipants = randstim1[, .N, by=participant]
    goodparticipants = goodparticipants[N == 2,]$participant
    
    randstim2 = randstim1[participant %in% goodparticipants]
    
    # find effect size
    ef = cohen.d(randstim2[old_object_condition == 3]$mean,
                 randstim2[old_object_condition == 1]$mean, paired = T)
    
    effect_list_random <- c(effect_list_random, ef$estimate)
  }
  return(effect_list_random)
}
