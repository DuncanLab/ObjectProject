...
soft = soft[!duplicated(soft$stimulus),]
...
We need to run the software again, because we didn't select for any of the orientation questions
...
ggplot(soft, aes(x = score_shoebox, fill = old_object_condition)) +
  geom_density(alpha=.3)
...
ggplot(combined_app_values, aes(x = score_shoebox, fill = group)) +
  geom_density(alpha = .5)

```


```{r run function - shoebox}

shoe_differences <- rand_diff_fun(variable = "score_shoebox")

shoe_differences_soft <- soft_diff_fun(variable = "score_shoebox")
```

```{r plot nameability}

shoe_differences$group = "Random"
setDT(shoe_differences)
shoe_differences = shoe_differences[, .(diff = mean(diff), CI_upper = CI(diff)[[1]], CI_lower= CI(diff)[[3]]), by = .(n, group)]

shoe_differences_soft$group = "Software"

plot_shoe = rbind(shoe_differences, shoe_differences_soft, fill = TRUE)

shoe_image <- ggplot(plot_shoe, aes(x = n, y = diff, color = group)) +
  geom_line() +
  ylab("mean shoebox score difference") +
  geom_errorbar(aes(ymin=CI_lower, ymax=CI_upper), width = .7) +
  ggtitle("Difference in stimuli shoebox score between conditions")

```

## Human-made

```{r run function - humanmade}

human_differences <- rand_diff_fun(variable = "score_humanmade")

human_differences_soft <- soft_diff_fun(variable = "score_humanmade")
```

```{r plot humanmade}

human_differences$group = "Random"
setDT(human_differences)
human_differences = human_differences[, .(diff = mean(diff), CI_upper = CI(diff)[[1]], CI_lower= CI(diff)[[3]]), by = .(n, group)]

human_differences_soft$group = "Software"

plot_human = rbind(human_differences, human_differences_soft, fill = TRUE)

human_image <- ggplot(plot_human, aes(x = n, y = diff, color = group)) +
  geom_line() +
  ylab("mean humanmade score difference") +
  geom_errorbar(aes(ymin=CI_lower, ymax=CI_upper), width = .7) +
  ggtitle("Difference in stimuli humanmade score between conditions")

```

## Outdoors

```{r run function - outdoors}

outdoors_differences <- rand_diff_fun(variable = "score_outdoors")

outdoors_differences_soft <- soft_diff_fun(variable = "score_outdoors")
```

```{r plot outdoors}

outdoors_differences$group = "Random"
setDT(outdoors_differences)
outdoors_differences = outdoors_differences[, .(diff = mean(diff), CI_upper = CI(diff)[[1]], CI_lower= CI(diff)[[3]]), by = .(n, group)]

outdoors_differences_soft$group = "Software"

plot_outdoors = rbind(outdoors_differences, outdoors_differences_soft, fill = TRUE)

outdoors_image <- ggplot(plot_outdoors, aes(x = n, y = diff, color = group)) +
  geom_line() +
  ylab("mean outdoors score difference") +
  geom_errorbar(aes(ymin=CI_lower, ymax=CI_upper), width = .7) +
  ggtitle("Difference in stimuli outdoors score between conditions")
