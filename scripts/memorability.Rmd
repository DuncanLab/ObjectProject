...
data_dir = "C:/Users/hanna/Documents/2019-2020/ObjectProject/datafiles/" #path where data is stored; depending on where you're running the script, we will need to tweak this
...
false_alarms_by_stimulus = recognition_by_stimulus[trialcode == "old_objects",
...
SS_hits_by_stimulus = split_sample_by_stimulus(data, num_sims = 1000, stat = "memory_hits") # 0.2980251

```

```{r Split samples for false alarm by stimulus}

SS_fa_by_stimulus = split_sample_by_stimulus(data, num_sims = 1000, stat = "memory_fa") # 0.3398046

```

```{r Split samples for dprime by stimulus}

SS_dprime_by_stimulus = split_sample_by_stimulus(data, num_sims = 1000, stat = "dprime") # 0.3196747

```

```{r Split samples for rt by stimulus}

SS_rt_by_stimulus = split_sample_by_stimulus(data, num_sims = 1000, stat = "rt") # 0.171698
...
SS_IES_by_stimulus = split_sample_by_stimulus(data, num_sims = 1000, stat = "IES") # 0.2867079
...
SS_corrected_hits_by_stimulus = split_sample_by_stimulus(data_corrected, num_sims = 100, stat = "memory_hits") # 0.3157219
...
SS_corrected_IES_by_stimulus = split_sample_by_stimulus(data_corrected, num_sims = 10, stat = "IES")
...
SS_IES_RTZ_by_stimulus = split_sample_by_stimulus(data_RTZ, num_sims = 1000, stat = "IES") # 0.2819942
...
SS_IES_corrhits_RTZ_by_stimulus = split_sample_by_stimulus(data_corrhits_RTZ, num_sims = 1000, stat = "IES") # 0.2829528
...
rmsubforsim <- function(tbl){
  ...
  cleaned_tbl = tbl[!participant %in% wide_summary[old_objects==0,]$participant]
...
plt_clean_extra+
  labs(title = "Cleaned dataset, extra batch only",
       subtitle = paste("sub per stim =", avgsub_clean_extra)) +
  geom_vline(xintercept = avgsub_clean_extra, col = "red") + xlim=c(0, 40)
...
pred_lowdprime <- function(tbl, min, max){
  ...
  pred_tbl = tbl[dprime %between% c(min, max), .(HR=mean(correct)), by=stimulus]
...
dprime_02_04 = pred_lowdprime(data_unclean, min=0.2, max=0.4)
...
mean(dprime_all[pred %between% c(-100, 100) & dprime < 0.8,]$pred)
...
mean(dprime_00_08[pred %between% c(-100, 100),]$pred)

dprime_all_threshold = pred_lowdprime(data_unclean, min=0, max=5)

mean(dprime_all_threshold[pred %between% c(-100, 100) & dprime < 0.8, ]$pred)
