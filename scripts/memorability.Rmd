...
rmsubforsim <- function(tbl){
  ...
  cleaned_tbl = tbl[!participant %in% wide_summary[old_objects<10,]$participant]
...
pred_lowdprime <- function(tbl, min, max){
  ...
  pred_tbl = tbl[dprime > cutoff_dprime, .(HR=mean(correct)), by=stimulus]
...
HR_ratio <- function(tbl, criteria){
  ...
  low_tbl = tbl[dprime < criteria, .(low_N = .N, HR_low=mean(correct)), by=.(stimulus, condition)]
  high_tbl = tbl[dprime > criteria, .(high_N = .N, HR_high=mean(correct)), by=.(stimulus, condition)]
  summary_tbl = tbl[, .(HR_all=mean(correct)), by=.(stimulus, condition)]
  
  merge_tbl = merge(low_tbl, high_tbl, by=c("stimulus","condition"))
  merge_tbl = merge(merge_tbl, summary_tbl, by=c("stimulus","condition"))
  merge_tbl[, ratio := HR_low / HR_high]
  
}

test = HR_ratio(test_data_unclean, .8)
